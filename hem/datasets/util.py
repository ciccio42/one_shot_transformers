...
def resize(image, target_dim, normalize=False, reduce_bits=False):
    ...
    if normalize:
        resized = (resized.astype(np.float32) / 255 - MEAN) / STD
        cv2.imwrite("normalized_resized.png", resized*255)
        return resized
...
def randomize_video(frames, color_jitter=None, rand_gray=None, rand_crop=None, rand_rot=0, rand_trans=np.array([0, 0]), normalize=False):
    ...
    if color_jitter is not None:
        rand_h, rand_s, rand_v = [
            np.random.uniform(-h, h) for h in color_jitter]
        delta = np.array([rand_h * 180, rand_s, rand_v]
                         ).reshape((1, 1, 3)).astype(np.float32)
        frames = [np.clip(cv2.cvtColor(cv2.cvtColor(
            fr, cv2.COLOR_RGB2HSV) + delta, cv2.COLOR_HSV2RGB), 0, 255) for fr in frames]
    if rand_gray and np.random.uniform() < rand_gray:
        frames = [np.tile(cv2.cvtColor(fr, cv2.COLOR_RGB2GRAY)[
                          :, :, None], (1, 1, 3)) for fr in frames]
    ...
    if rand_rot or any(rand_trans):
        ...
        M = np.array([[np.cos(rot), -np.sin(rot), trans[0]],
                     [np.sin(rot), np.cos(rot), trans[1]]])
        frames = [cv2.warpAffine(fr, M, (fr.shape[1], fr.shape[0]))
                  for fr in frames]
    if normalize:
